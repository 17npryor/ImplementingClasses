...
class Point(object):
    ...
    def move_to(self, x,y):
        self.dist = math.sqrt(((x - self.x) ** (2)) + ((y - self.y) ** (2)))
        self.x = x
        self.y = y
        self.total_moves = self.total_moves + 1
        self.gettotaldistance = self.gettotaldistance + self.dist

    def move_by(self,dx,dy):
        self.x = self.x + dx
        self.y = self.y + dy
        self.total_moves = self.total_moves + 1
        self.gettotaldistance = self.gettotaldistance + math.sqrt((dx)**(2) + (dy)**(2))
